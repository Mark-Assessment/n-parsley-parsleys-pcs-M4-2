...
def add_to_cart(request, product_id):
    ...
    if request.user.is_authenticated:
        cart_item, created = CartItem.objects.get_or_create(
            user=request.user, product=product, purchased=False)
    else:
        ...
        if not session_key:
            ...
            session_key = request.session.session_key
        cart_item, created = CartItem.objects.get_or_create(
            session_key=session_key, product=product, purchased=False)
...
def merge_cart_items(sender, request, user, **kwargs):
    ...
    for session_item in session_cart_items:
        user_item, created = user_cart_items.get_or_create(
